...
export default function EditAnalysis({ id }: EditAnalysisProps) {
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 404) {
            toast.error("تحلیل مورد نظر یافت نشد")
        ...
        toast.error("خطا در بارگذاری اطلاعات تحلیل")
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("تحلیل با موفقیت بروزرسانی شد")
      router.push("/user/analyses")
    } catch (error) {
      console.error("Error updating analysis:", error)
      toast.error("خطا در بروزرسانی تحلیل")
    } finally {
      setIsSubmitting(false)
    }
  }

  if (isLoading || !isAuthenticated || isLoadingAnalysis) {
    return (
  ...
  if (!isLoaded) {
    return (
      <div className="flex flex-col items-center justify-center h-[50vh]">
        <p className="text-lg text-muted-foreground mb-4">تحلیل مورد نظر یافت نشد</p>
        <Button onClick={() => router.push("/user/analyses")}>بازگشت به لیست تحلیل‌ها</Button>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <Card>
        <CardHeader>
          <CardTitle>ویرایش تحلیل</CardTitle>
          <CardDescription>اطلاعات تحلیل خود را ویرایش کنید.</CardDescription>
        </CardHeader>
