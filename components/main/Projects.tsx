  ...
  background: ${({ status }) =>
    status === "approved"
      ...
      : status === "rejected"
        ...
        : "linear-gradient(45deg, #f7b733, #fc4a1a)"};
  color: #fff;
  padding: 3px 10px;
  border-radius: 4px;
  font-size: 12px;
...
// Component for a single project box
const ProjectBox = ({
  project,
  index,
}: {
  ...
  return (
    <Box delay={index * 0.1}>
      <StatusBadge status={project.status}>{getStatusText(project.status)}</StatusBadge>
      <Title>{project.platform_name}</Title>
      <UserInfoContainer>
        <UserImage
          src={project.user?.image || `/placeholder.svg?height=40&width=40&query=user avatar`}
          alt="User Avatar"
        />
        <UserName>{project.user?.name || "کاربر پلاس چارت"}</UserName>
      </UserInfoContainer>
      <DateText>تاریخ: {formattedDate}</DateText>
      <Description>{project.user_description}</Description>

      {project.admin_description && (
        <Description style={{ marginTop: "15px", color: "#ffd700" }}>
          نظر کارشناس: {project.admin_description}
        </Description>
      )}
      ...
      <ButtonContainer>
        <WebsiteButton
          as="a"
          href={formatWebsiteUrl(project.platform_website)}
          target="_blank"
          rel="noopener noreferrer"
        >
          <WebsiteIcon>🌐</WebsiteIcon>
          {displayUrl(project.platform_website)}
        </WebsiteButton>
      </ButtonContainer>
    </Box>
  )
...
const Projects = () => {
  ...
  const fetchProjects = async () => {
    ...
    try {
      ...
      setError("خطا در بارگذاری پروژه‌ها. لطفا دوباره تلاش کنید.")
    } finally {
      setLoading(false)
    }
  }
    ...
    <>
      ...
      <PageContainer>
        ...
        <TitleContainer>
          ...
          <SectionTitle>پروژه‌های کاربران</SectionTitle>
          <DecorativeLine direction="right" />
        </TitleContainer>

        <Subtitle data-text="پروژه‌های کاربران">پروژه‌های بررسی شده توسط کارشناسان پلاس چارت</Subtitle>
        ...
        {loading ? (
          ...
          <NoResultsContainer>
            <NoResultsText>هیچ پروژه‌ای یافت نشد</NoResultsText>
          </NoResultsContainer>
        ) : (
          <Container>
        ...
        {!loading && projects.length > 0 && pagination.pages > 1 && (
          <PaginationContainer>
